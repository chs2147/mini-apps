...
- **Exclusive 카테고리 정책**: 이 저장소 밖의 외부 링크(다른 도메인에 호스팅된 앱)를 추가할 때는 `apps-data.js`에서 `category: 'exclusive'`, `external: true`를 지정한다.
  ...
  - **신규 앱 추가 요청에 지원 스펙(데스크탑/모바일 등 devices)이 누락되어 있으면, 임의로 추정하거나 비워두고 넘어가지 말고 앱을 추가하기 전에 반드시 사용자에게 먼저 질문한다.**
  - 기기 지원 여부(`devices`)와 별개로 **앱 자체의 기능/특성**(예: 온라인 대전)은 `features` 배열에 담아 `FEATURE_META`에 정의하고 `featureBadgesHtml()`로 렌더링한다. device-badge와는 다른 강조 색상(시안 계열)을 써서 "어디서 도는지"와 "무엇을 지원하는지"가 한눈에 구분되게 한다. 새 기능 태그가 필요하면 `FEATURE_META`에 항목을 추가할 것.
