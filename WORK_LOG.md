...
| 2026-07-14 | 랜딩 페이지 헤더에 외부 무료 카운터 서비스(`api.counterapi.dev`) 기반 누적 방문자 수 뱃지 추가. 백엔드가 없는 정적 사이트라 진짜 누적 카운트를 모든 방문자에게 동일하게 보여주려면 외부 서비스가 불가피함 — `api.countapi.xyz`는 서비스 중단 확인되어 제외, counterapi.dev로 대체. 실패 시 뱃지를 조용히 숨기는 fallback 처리 |
